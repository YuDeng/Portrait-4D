...
def optimize():
    ...
    # load subfolders
    all_sub = sorted([s for s in os.listdir(data_dir) if "2dldmks_align" in os.listdir(os.path.join(data_dir,s)) and 'bfm2flame_params' in os.listdir(os.path.join(data_dir,s))])
    print('remain num:',len(all_sub))

    n_sub = 1
    # optimize in parallel for n_sub
    for sub_idx in range(0,len(all_sub),n_sub):
        current_sub = all_sub[sub_idx:sub_idx+n_sub]
        current_data_dir = []
        current_save_dir = []
        current_data_list = []
        current_ldmks_2d_list = []
        current_ldmks_3d_list = []

        for sub in current_sub:
            sub_dir = os.path.join(data_dir,sub)
            save_sub_dir = os.path.join(save_dir,sub)
            os.makedirs(save_dir,exist_ok=True)
            
            data_list = sorted([os.path.join(sub_dir,'align_images',f) for f in os.listdir(os.path.join(sub_dir,'align_images')) if f.endswith('png') or f.endswith('jpg')])
            frame_len = len(data_list)
            if not args.batchsize == -1:
                frame_len = min(frame_len, args.batchsize)
            print('frame_len:',frame_len)
            select_idx = list(range(0,frame_len))
            data_list = [data_list[i] for i in select_idx]
            current_data_list += data_list
            current_data_dir += [sub_dir]*frame_len
            current_save_dir += [save_sub_dir]*frame_len
            ...
            for idx, img_path in enumerate(data_list):
                ...
                ldmks_2d_path = os.path.join(sub_dir, '2dldmks_align', img_name+'.npy')
                ldmks_3d_path = os.path.join(sub_dir, '3dldmks_align', img_name+'.npy')
            ...
            ldmks_2d_list = np.stack(ldmks_2d_list,0).reshape(-1,98,2)
            ldmks_3d_list = np.stack(ldmks_3d_list,0).reshape(-1,68,2)
            ldmks_2d_list = ldmks_2d_list[select_idx]
            ldmks_3d_list = ldmks_3d_list[select_idx]

            current_ldmks_2d_list.append(ldmks_2d_list)
            current_ldmks_3d_list.append(ldmks_3d_list)
        
        current_ldmks_2d_list = np.concatenate(current_ldmks_2d_list,0)
        current_ldmks_3d_list = np.concatenate(current_ldmks_3d_list,0)
        ldmks_2d = current_ldmks_2d_list
        ldmks_3d = current_ldmks_3d_list
        
        current_param_list = [f.replace('align_images','bfm2flame_params').replace('.jpg','.npy').replace('.png','.npy') for f in current_data_list]


        # hyper-parameters for optimization
        flen = len(current_data_list)
        batchsize = flen
        n_iter_sec1 = 1500
        n_iter_sec2 = 3000
        image_size = (512, 512)
        ...
        for i in range(0, flen, batchsize):
            ...
            for k in range(min(batchsize,flen-i)):
                params_bfm2flame = np.load(current_param_list[i+k], allow_pickle=True).item()
            ...
            cur_shape_params = cur_shape_params.reshape(n_sub,frame_len,-1)
            cur_shape_params = torch.mean(cur_shape_params,dim=1,keepdim=False) # (n_sub, s_dim)
            ...
            t = torch.zeros(1, 3).float().cuda().repeat(cur_exp_params.shape[0], 1)
            ...
            for ii in range(n_iter_sec1):
                ...
                expand_optim_shape_params = optim_shape_params.unsqueeze(1).repeat(1,frame_len,1).reshape(n_sub*frame_len,-1)
                cur_shape, _, cur_3ds, joints = flamedeform.flame_model(expand_optim_shape_params, optim_exp_params, torch.cat([torch.zeros_like(cur_pose_params[:,:3]), optim_jaw_pose_params], 1), torch.cat([optim_eye_pose_params, optim_eye_pose_params], 1))
            ...
            for ii in range(n_iter_sec2):
                ...
                expand_optim_shape_params = optim_shape_params.unsqueeze(1).repeat(1,frame_len,1).reshape(n_sub*frame_len,-1)
                cur_shape, _, cur_3ds, joints = flamedeform.flame_model(expand_optim_shape_params, optim_exp_params, torch.cat([torch.zeros_like(cur_pose_params[:,:3]), optim_jaw_pose_params], 1), torch.cat([optim_eye_pose_params, optim_eye_pose_params], 1))
                expand_init_shape_params = init_shape_params.unsqueeze(1).repeat(1,frame_len,1).reshape(n_sub*frame_len,-1)
                init_shape, _, _, _ = flamedeform.flame_model(expand_init_shape_params, init_exp_params, torch.cat([torch.zeros_like(cur_pose_params[:,:3]), optim_jaw_pose_params], 1), torch.cat([optim_eye_pose_params, optim_eye_pose_params], 1))
            ...
            # repeat shape parameter
            expand_cur_shape_min = cur_shape_min.unsqueeze(1).repeat(1,frame_len,1).reshape(n_sub*frame_len,-1)
            
            # finalize optimized flame parameters
            cur_labels = torch.cat([c_compose,expand_cur_shape_min.detach(),cur_exp_min,torch.cat([cur_pose_params[:,:3],cur_jawpose_min],dim=1),torch.cat([cur_eyepose_min, cur_eyepose_min], 1)],dim=-1).cpu().numpy()
            
            if args.visualize:
                with torch.no_grad():
                    uv_compose = flamedeform.renderer(expand_cur_shape_min.clone().detach(),cur_exp_min.clone().detach(),torch.cat([torch.zeros_like(cur_pose_params[:,:3]),cur_jawpose_min],dim=1),torch.cat([cur_eyepose_min, cur_eyepose_min], 1),c_compose, fov=12, half_size=256)[0]

                    uv_head = []
                    uv_mask = []
                    for k in range(len(expand_cur_shape_min)):
                        uv_head_, uv_mask_ = flamedeform.renderer_visualize(expand_cur_shape_min[k:k+1].clone().detach(),cur_exp_min[k:k+1].clone().detach(),torch.cat([torch.zeros_like(cur_pose_params[k:k+1,:3]),cur_jawpose_min[k:k+1]],dim=1),torch.cat([cur_eyepose_min[k:k+1], cur_eyepose_min[k:k+1]], 1),c_compose[k:k+1], fov=12, half_size=256)
                        uv_head.append(uv_head_)
                        uv_mask.append(uv_mask_)
                    uv_head = torch.cat(uv_head,dim=0)
                    uv_mask = torch.cat(uv_mask,dim=0)

                    _, face_wo_eye_mask = flamedeform.renderer_visualize(expand_cur_shape_min.clone().detach(),cur_exp_min.clone().detach(),torch.cat([torch.zeros_like(cur_pose_params[:,:3]),cur_jawpose_min],dim=1),torch.cat([cur_eyepose_min, cur_eyepose_min], 1),c_compose, fov=12, half_size=256,face_woeye=True,eye_mask=False)
                    _, eye_mask = flamedeform.renderer_visualize(expand_cur_shape_min.clone().detach(),cur_exp_min.clone().detach(),torch.cat([torch.zeros_like(cur_pose_params[:,:3]),cur_jawpose_min],dim=1),torch.cat([cur_eyepose_min, cur_eyepose_min], 1),c_compose, fov=12, half_size=256,face_woeye=False,eye_mask=True)
                    
                    eye_mask = eye_mask * (1-face_wo_eye_mask)

            for j in range(len(cur_labels)):

                img_path = current_data_list[i+j]
                img_name = img_path.split("/")[-1].split(".")[0]
                save_label_dir = os.path.join(current_save_dir[i+j], "flame_optim_params")
                os.makedirs(save_label_dir, exist_ok=True)
                np.save(os.path.join(save_label_dir,'{}.npy'.format(img_name)), cur_labels[j])
                
                if args.visualize:
                
                    save_uv_dir = os.path.join(current_save_dir[i+j], "flame_optim_uvs")
                    save_show_dir = os.path.join(current_save_dir[i+j], "flame_optim_visuals")
