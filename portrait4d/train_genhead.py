# Training script of GenHead, modified from EG3D: https://github.com/NVlabs/eg3d
...
import dnnlib
from training import training_loop_genhead as training_loop
...
def init_dataset_kwargs(data):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataloader.dataset.PortraitSynthesisSegLmdbFolderDatasetV2', resolution=512, data_type='ffhq', path=data, use_labels=True, max_size=None, xflip=False)
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=512, w_dim=512, mapping_kwargs=dnnlib.EasyDict(), deformation_kwargs=dnnlib.EasyDict(flame_cfg_path='models/FLAME/cfg.yaml', mapping_kwargs=dnnlib.EasyDict(),box_warp=1.3))
    ...
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.loss_genhead.PortraitSynthesisLoss')
    ...
    c.G_kwargs.flame_condition = opts.g_flame_condition
    c.G_kwargs.random_combine = opts.g_random_combine
    c.G_kwargs.dynamic_texture = opts.g_dynamic_texture
    ...
    c.G_kwargs.sr_kwargs = dnnlib.EasyDict(channel_base=opts.cbase, channel_max=opts.cmax, fused_modconv_default='inference_only', use_ws_ones=True)

    c.loss_kwargs.style_mixing_prob = opts.style_mixing_prob

    # Augmentation.
    if opts.aug != 'noaug':
        c.augment_kwargs = dnnlib.EasyDict(class_name='training.dataloader.augment.AugmentPipe', xflip=1, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1, brightness=1, contrast=1, lumaflip=1, hue=1, saturation=1)
        if opts.aug == 'ada':
