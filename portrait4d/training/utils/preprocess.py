# Face image preprocessing code, modified from Deep3DFaceRecon_pytorch: https://github.com/sicxu/Deep3DFaceRecon_pytorch 
...
def extract_5p_flame(lm):
    lm5p = lm[[68,69,30,48,54], :]
    return lm5p
...
# utils for pd-fgc motion extractor
...
# pytorch version of estimate_norm
def estimate_norm_torch(lm_70p, H):
    lm_70p_ = lm_70p.detach().cpu().numpy()
    M = []
    for i in range(lm_70p_.shape[0]):
        M.append(estimate_norm(lm_70p_[i], H))
    M = torch.tensor(np.array(M), dtype=torch.float32).to(lm_70p.device)
    return M

# pytorch version of estimate_norm_pdfgc
