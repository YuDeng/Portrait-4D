# Training script of Portrait4D-v2, modified from EG3D: https://github.com/NVlabs/eg3d
...
import dnnlib
from training import training_loop_recon_v2 as training_loop
...
def init_dataset_kwargs(data, class_name='training.dataloader.dataset_recon.OneShotReconSegLmdbFolderDataset'):
    try:
...
def main(**kwargs):
    ...
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.loss_recon_v2.AnimatablePortraitReconLoss')
    ...
    if opts.max_num == -1:
        c.training_set_kwargs.max_num = None
    else:
        c.training_set_kwargs.max_num = opts.max_num
    ...
    c.loss_kwargs.mot_aug_prob = opts.mot_aug_prob
    c.loss_kwargs.G_fix_reg_img_aug = opts.g_fix_reg_img_aug
    ...
    if opts.resume_fix is not None:
        c.resume_pkl_G_fix = opts.resume_fix
