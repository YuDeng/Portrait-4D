...
@click.option('--data', help='Dataset to evaluate against  [default: look up]', metavar='[ZIP|DIR]', default='./data/FFHQ_512_50/')
...
@click.option('--reload_modules', help='Print optional information', type=bool, default=False, metavar='BOOL', show_default=True)

def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose, render_size, chunk, reload_modules):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    ...
    if reload_modules:
        ...
        # G.init_kwargs.deformation_kwargs.flame_full = True
        use_rotation_limits = False
        G_new = PartTriPlaneGeneratorDeform(*args.G.init_args, **args.G.init_kwargs, flame_full=use_rotation_limits).eval().requires_grad_(False)
        misc.copy_params_and_buffers(args.G, G_new, require_all=False)
        G_new.neural_rendering_resolution =args.G.neural_rendering_resolution
        G_new.rendering_kwargs = args.G.rendering_kwargs
        args.G = G_new

    # Initialize dataset options.
    if data is not None:
        args.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataloader.dataset.PortraitSynthesisSegLmdbFolderDatasetV2', resolution=512, data_type='ffhq', path=data, use_labels=True, max_size=None, xflip=False)
    ...
    args.metric_kwargs = dnnlib.EasyDict(
        neural_rendering_resolution=render_size,
        chunk=(None, chunk)[render_size>64],
        )
