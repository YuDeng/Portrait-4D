...
class LookAtPoseSampler:
    ...
    @staticmethod
    def sample(horizontal_mean, vertical_mean, lookat_position, horizontal_stddev=0, vertical_stddev=0, roll_mean=0, roll_stddev=0, radius=1, batch_size=1, device='cpu'):
        h = torch.randn((batch_size, 1), device=device) * horizontal_stddev + horizontal_mean
        v = torch.randn((batch_size, 1), device=device) * vertical_stddev + vertical_mean
        v = torch.clamp(v, 1e-5, math.pi - 1e-5)
        ...
        theta = h
        v = v / math.pi
        phi = torch.arccos(1 - 2*v) + math.pi
        ...
        up_vectors = torch.tensor([0, 1, 0], dtype=torch.float, device=device).expand_as(forward_vectors)
        right_vectors = -math_utils.normalize_vecs(torch.cross(up_vectors, forward_vectors, dim=-1))
        up_vectors = math_utils.normalize_vecs(torch.cross(right_vectors, forward_vectors, dim=-1))

        r = torch.randn((batch_size, 1), device=device) * roll_stddev + roll_mean
        up_vectors = up_vectors*torch.cos(r) + right_vectors*torch.sin(r)

        return create_cam2world_matrix(forward_vectors, camera_origins, up_vector=up_vectors)
...
def create_cam2world_matrix(forward_vector, origin, up_vector=None): # add up_vector by Dengyu following GRAMInverter
    """
